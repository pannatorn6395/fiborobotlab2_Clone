...
class Camera():
    def __init__(self, i=0, width=1280, height=720):
        if os.name == 'nt':
...
class CameraPublisher(Node):
    def __init__(self):
        super().__init__('camera_publisher_{}'.format(robotname))
        ...
        image_topic_name = '/camera{}/image_{}'.format(camera_name,robotname)              # Publish image to /cameraX/image
        self.publisher_ = self.create_publisher(Image, image_topic_name, 2)
        ...
        timer_period = 1/90  # seconds
        self.timer = self.create_timer(timer_period, self.timer_callback)
        config = simplejson.load(open(os.path.join(get_package_share_directory('fiborobotlab2'), 'config', 'camera_config.json')))
