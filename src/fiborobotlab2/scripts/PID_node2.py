...
class PID_node(Node):
    def __init__(self):
        ...
        self.publisher_ = self.create_publisher(SetPosition, 'set_position_robot1', 10)     # CHANGE
        self.cli = self.create_client(GetPosition, 'get_position_robot1')
        self.subscription = self.create_subscription(Rstate,'Ball_position_robot1',self.Calculate_PID,10)
        
        self.publisher_2 = self.create_publisher(Rstate, 'action_state_robot1', 10) 
        self.publisher_3 = self.create_publisher(Rstate, 'Head_position_robot1', 10) 
        # timer_period = 0.5
            ...
            if count_negative>2:
                if PID.robot_state=="scanball":
                    ...
                    try:
                    	...
                    	PID.get_logger().info('Service call failed %r' % (e,))
                    else:
                    	presentposition=response.position
                    if count_pan==0:
                        count_pan=0   
